...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  closed: { label: "Kapalı", color: "bg-red-500/10 text-red-500" },
};
...
const statusFilters = [
  { value: "all", label: "Tüm Durumlar", count: 2847 },
  { value: "active", label: "Aktif", count: 2634, color: "text-green-500" },
  { value: "suspended", label: "Askıda", count: 156, color: "text-orange-500" },
  { value: "pending", label: "Beklemede", count: 57, color: "text-yellow-500" },
  { value: "closed", label: "Kapalı", count: 57, color: "text-red-500" },
];

const typeFilters = [
  { value: "all", label: "Tüm Tipler", count: 2847 },
  { value: "bireysel", label: "Bireysel", count: 2000, color: "text-cyan-500" },
  { value: "kurumsal", label: "Kurumsal", count: 847, color: "text-orange-500" },
];
    ...
    <div className="space-y-6">
      ...
      <BlurFade delay={0}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Tüm Müşteriler</h1>
            <p className="text-zinc-600 dark:text-zinc-400">Sistemdeki tüm müşterilerin listesi</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-500/10">
            <Users className="h-5 w-5 text-blue-500" />
            <span className="text-sm font-medium text-blue-500">2,847 Müşteri</span>
          </div>
        </div>
      </BlurFade>
      ...
      <BlurFade delay={0.3}>
        <div className="flex flex-wrap gap-2">
          {typeFilters.map((filter) => (
            ...
            >
              {filter.value === "bireysel" && <UserCheck className="mr-1.5 h-3.5 w-3.5" />}
              {filter.value === "kurumsal" && <Building2 className="mr-1.5 h-3.5 w-3.5" />}
              {filter.label}
              <span className={cn(
                "ml-2 px-1.5 py-0.5 rounded text-xs",
                typeFilter === filter.value
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-200 dark:border-zinc-800">
                    <th className="text-left p-4 text-sm font-medium text-zinc-600 dark:text-zinc-400">Adı Soyadı / Firma</th>
                    ...
                    <tr>
                      <td colSpan={7} className="p-8 text-center">
                        <div className="flex flex-col items-center gap-2">
                          <UserXIcon className="h-12 w-12 text-zinc-300 dark:text-zinc-700" />
